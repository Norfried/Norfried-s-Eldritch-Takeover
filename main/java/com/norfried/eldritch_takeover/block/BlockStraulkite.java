...
public class BlockStraulkite extends Block implements IHasModel {

    private final int ordinal;


    public BlockStraulkite(Material material, float hardness, float resistance, SoundType soundType, int ordinal) {
        super(material);
        this.setHardness(hardness);
        this.setResistance(resistance);
        this.setSoundType(soundType);
        this.setCreativeTab(Main.etcreativetab);
    ...
    public void neighborChanged(IBlockState state, World world, BlockPos pos, Block block, BlockPos from)
    {
        ...
        if (facesLost > 0)
        {
            ...
            //    spawnAsEntity(world, pos, new ItemStack(this, facesLost));
        }
    }
    ...
    @Override
    public MapColor getMapColor(IBlockState state, IBlockAccess world, BlockPos pos) { return MapColor.BLUE_STAINED_HARDENED_CLAY; }

    @Override
    public ItemStack getPickBlock(IBlockState state, RayTraceResult target, World world, BlockPos pos, EntityPlayer player) { return new ItemStack(Item.getItemFromBlock(this)); }
    ...
    {
        if (!(state.getBlock() instanceof BlockStraulkite)) return 0;
        return ((BlockStraulkite) state.getBlock()).getOrdinal() * 16 + state.getValue(META);
    }

    //   public static IBlockState getBlockState(int meta)
    //   { return meta == 0 ? Blocks.AIR.getDefaultState() : ModBlocks.BlockStraulkite[meta / 16].getDefaultState().withProperty(META, meta % 16); }
